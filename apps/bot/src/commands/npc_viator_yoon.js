 ...
 * COMANDO: !viator_yoon
 * 
 * Exibe a ficha completa do NPC Viator Yoon.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("viator_yoon");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!viator_yoon ficha" || texto === "!viator_yoon ficha completa") {
        let mensagem = `*═══ FICHA DE VIATOR YOON ═══*
`;
        mensagem += `──────────────────────────
