 ...
 * COMANDO: !rondo_baek
 * 
 * Exibe a ficha completa do NPC Rondo Baek.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("rondo_baek");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!rondo_baek ficha" || texto === "!rondo_baek ficha completa") {
        let mensagem = `*═══ FICHA DE RONDO BAEK ═══*
`;
        mensagem += `──────────────────────────
