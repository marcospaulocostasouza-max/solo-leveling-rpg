...
/**
 * COMANDO: !entidade_mae
 * 
 * Exibe a ficha completa do NPC Entidade 'Mãe'.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("entidade_mae");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!entidade_mae ficha" || texto === "!entidade_mae ficha completa") {
        let mensagem = `*═══ FICHA DE ENTIDADE 'MÃE' ═══*
`;
        mensagem += `──────────────────────────
