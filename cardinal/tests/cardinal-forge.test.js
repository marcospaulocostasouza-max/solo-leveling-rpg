...
const { CardinalForgeService, ForgeValidator, CODES, ForgeRegistry, autoCompletedFields, prepareContent } = require("../forge");
const { structuredEnvelope, applyExplicitConstraints } = require("../forge/base-forge");
...
const weapon = (overrides = {}) => ({ nome: "Lâmina Solar", categoria: "Arma", slot: "Arma 1", tier: "A", descricao: "A lâmina possui aço dourado gravado com runas solares ao longo do fio. O guarda-mão largo protege a empunhadura durante avanços rápidos. Ela foi feita para duelos contra criaturas das sombras. Sua luz acompanha cortes precisos e deixa o alcance visual mais claro.", forca_bonus: 20, resistencia_bonus: 0, velocidade_bonus: 0, sentidos_bonus: 0, inteligencia_bonus: 0, poder_magico_bonus: 0, efeito: "Emite luz.", ...overrides });
...
test("Cardinal Forge: geração, validação, versões e segurança", async t => {
    ...
    await t.test("qualidade de item exige descrição, efeito e nome utilizáveis", async () => { const validation = await validator.validate("weapon", weapon({ nome: "Espada", descricao: "Uma espada poderosa.", efeito: "" })); assert.ok(validation.errors.some(error => error.code === CODES.QUALITY_INCOMPLETE && error.field === "descricao")); assert.ok(validation.errors.some(error => error.code === CODES.QUALITY_INCOMPLETE && error.field === "efeito")); assert.ok(validation.errors.some(error => error.code === CODES.QUALITY_INCOMPLETE && error.field === "nome")); });
    ...
    await t.test("pedido de atributo específico não é trocado por outro", async () => { const output = await service.generate("Crie uma arma Rank D com 10 de atributo em força", { type: "weapon", content: weapon({ nome: "Kunai", tier: "D", forca_bonus: 0, velocidade_bonus: 10 }), author: "tester" }); assert.ok(output.draft.validation.errors.some(error => error.field === "forca_bonus" && error.code === CODES.CONSTRAINT_MISMATCH)); });
