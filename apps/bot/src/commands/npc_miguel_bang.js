 ...
 * COMANDO: !miguel_bang
 * 
 * Exibe a ficha completa do NPC Miguel Bang.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("miguel_bang");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!miguel_bang ficha" || texto === "!miguel_bang ficha completa") {
        let mensagem = `*═══ FICHA DE MIGUEL BANG ═══*
`;
        mensagem += `──────────────────────────
