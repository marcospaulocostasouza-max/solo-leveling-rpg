...
const BANNER = [
  ...
  ["conjuntos", "Quais *conjuntos* entram no Banner? Separe nomes por vírgula."], ["titulo", "Qual *Título raro* será entregue?"],
  ["passivas", "Quais *Passivas raras* serão entregues? Separe por vírgula."], ["itemSecreto", "Qual é o *Item Secreto* do Banner?"],
  ["inicioEm", "Informe a *data inicial* no formato DD/MM/AAAA HH:mm."], ["fimEm", "Informe a *data final* no formato DD/MM/AAAA HH:mm."]
];
const DUNGEON = [
  ...
  ["won", "Qual é a recompensa em *Won*?"], ["itens", "Quais são os *itens de recompensa*? Escreva Nenhum se não houver."], ["regras", "Quais são as *regras específicas*? Escreva Regras gerais se não houver."], ["duracao", "Qual é a *duração/período disponível*?"]
];

let ready;
async function ensure() {
  if (!ready) ready = database.run("CREATE TABLE IF NOT EXISTS admin_creation_sessions (administrador TEXT PRIMARY KEY,tipo TEXT NOT NULL,etapa INTEGER NOT NULL DEFAULT 0,dados TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'COLETANDO',atualizado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)");
  return ready;
}
...
function validateDungeon(data) {
  const errors = []; for (const key of ["nome", "tema", "rank", "descricao", "objetivo", "participantes", "xp", "won", "duracao"]) if (!String(data[key] || "").trim()) errors.push(`${key} é obrigatório.`);
  if (data.rank && !["E", "D", "C", "B", "A", "S"].includes(String(data.rank).trim().toUpperCase())) errors.push("Rank deve ser E, D, C, B, A ou S.");
  for (const key of ["participantes", "xp", "won"]) if (data[key] != null && (!Number.isSafeInteger(Number(data[key])) || Number(data[key]) < 0)) errors.push(`${key} deve ser um número inteiro não negativo.`);
  return errors;
}
async function answer(actor, text) {
  ...
  const [field] = questions(state.tipo)[state.etapa]; state.dados[field] = value;
  state.etapa += 1;
  ...
  const errors = validateDungeon(state.dados);
  if (errors.length) { state.etapa = 0; await save(actor, state); return { consumed: true, text: `A Dungeon possui problemas:\n${errors.map(item => `• ${item}`).join("\n")}\n\nVamos reiniciar. ${questions("DUNGEON")[0][1]}` }; }
  state.status = "AGUARDANDO_IMAGEM"; await save(actor, state);
  return { consumed: true, text: `_*「 DUNGEON SEMANAL VALIDADA 」*_\n\nAgora envie a imagem com a legenda:\n*!anexar imagem dungeon semanal ${state.dados.nome}*` };
}
async function attachImage(actor, tipo, nome, media) {
  const state = await get(actor); if (!state || state.tipo !== tipo || state.status !== "AGUARDANDO_IMAGEM") throw new Error("Não há uma criação sua aguardando imagem.");
  if (String(tipo === "BANNER" ? state.dados.bannerNome : state.dados.nome).normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase() !== String(nome || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()) throw new Error("O nome não corresponde à criação que aguarda imagem.");
