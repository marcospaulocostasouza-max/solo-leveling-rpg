...
/**
 * COMANDO: !elrica_edoras
 * 
 * Exibe a ficha completa do NPC Elrica Edoras.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("elrica_edoras");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!elrica_edoras ficha" || texto === "!elrica_edoras ficha completa") {
        let mensagem = `*═══ FICHA DE ELRICA EDORAS ═══*
`;
        mensagem += `──────────────────────────
