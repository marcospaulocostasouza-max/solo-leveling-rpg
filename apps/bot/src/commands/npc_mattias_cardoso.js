...
/**
 * COMANDO: !mattias_cardoso
 * 
 * Exibe a ficha completa do NPC Mattias Cardoso.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("mattias_cardoso");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!mattias_cardoso ficha" || texto === "!mattias_cardoso ficha completa") {
        let mensagem = `*═══ FICHA DE MATTIAS CARDOSO ═══*
`;
        mensagem += `──────────────────────────
