 ...
 * COMANDO: !rufus_deng
 * 
 * Exibe a ficha completa do NPC Rufus Deng.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("rufus_deng");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!rufus_deng ficha" || texto === "!rufus_deng ficha completa") {
        let mensagem = `*═══ FICHA DE RUFUS DENG ═══*
`;
        mensagem += `──────────────────────────
