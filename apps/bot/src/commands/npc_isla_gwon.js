 ...
 * COMANDO: !isla_gwon
 * 
 * Exibe a ficha completa do NPC Isla Gwon.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("isla_gwon");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!isla_gwon ficha" || texto === "!isla_gwon ficha completa") {
        let mensagem = `*═══ FICHA DE ISLA GWON ═══*
`;
        mensagem += `──────────────────────────
