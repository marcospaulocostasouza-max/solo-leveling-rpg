 ...
 * COMANDO: !eltrix_noh
 * 
 * Exibe a ficha completa do NPC Eltrix Noh.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("eltrix_noh");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!eltrix_noh ficha" || texto === "!eltrix_noh ficha completa") {
        let mensagem = `*═══ FICHA DE ELTRIX NOH ═══*
`;
        mensagem += `──────────────────────────
