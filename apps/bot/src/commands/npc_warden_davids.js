...
/**
 * COMANDO: !warden_davids
 * 
 * Exibe a ficha completa do NPC Warden Davids.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("warden_davids");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!warden_davids ficha" || texto === "!warden_davids ficha completa") {
        let mensagem = `*═══ FICHA DE WARDEN DAVIDS ═══*
`;
        mensagem += `──────────────────────────
