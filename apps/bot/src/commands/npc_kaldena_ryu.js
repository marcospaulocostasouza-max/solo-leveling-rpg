 ...
 * COMANDO: !kaldena_ryu
 * 
 * Exibe a ficha completa do NPC Kaldena Ryu.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("kaldena_ryu");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!kaldena_ryu ficha" || texto === "!kaldena_ryu ficha completa") {
        let mensagem = `*═══ FICHA DE KALDENA RYU ═══*
`;
        mensagem += `──────────────────────────
