 ...
 * COMANDO: !ori_choi
 * 
 * Exibe a ficha completa do NPC Ori Choi.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("ori_choi");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!ori_choi ficha" || texto === "!ori_choi ficha completa") {
        let mensagem = `*═══ FICHA DE ORI CHOI ═══*
`;
        mensagem += `──────────────────────────
