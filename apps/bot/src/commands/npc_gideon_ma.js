 ...
 * COMANDO: !gideon_ma
 * 
 * Exibe a ficha completa do NPC Gideon Ma.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("gideon_ma");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!gideon_ma ficha" || texto === "!gideon_ma ficha completa") {
        let mensagem = `*═══ FICHA DE GIDEON MA ═══*
`;
        mensagem += `──────────────────────────
