 ...
 * COMANDO: !helgenish
 * 
 * Exibe a ficha completa do NPC Helgenish.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("helgenish");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!helgenish ficha" || texto === "!helgenish ficha completa") {
        let mensagem = `*═══ FICHA DE HELGENISH ═══*
`;
        mensagem += `──────────────────────────
