 ...
 * COMANDO: !solon_wi
 * 
 * Exibe a ficha completa do NPC Solon Wi.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("solon_wi");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!solon_wi ficha" || texto === "!solon_wi ficha completa") {
        let mensagem = `*═══ FICHA DE SOLON WI ═══*
`;
        mensagem += `──────────────────────────
