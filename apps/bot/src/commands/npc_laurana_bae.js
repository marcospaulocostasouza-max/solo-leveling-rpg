 ...
 * COMANDO: !laurana_bae
 * 
 * Exibe a ficha completa do NPC Laurana Bae.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("laurana_bae");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!laurana_bae ficha" || texto === "!laurana_bae ficha completa") {
        let mensagem = `*═══ FICHA DE LAURANA BAE ═══*
`;
        mensagem += `──────────────────────────
