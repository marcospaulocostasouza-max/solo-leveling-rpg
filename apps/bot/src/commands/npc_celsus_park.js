 ...
 * COMANDO: !celsus_park
 * 
 * Exibe a ficha completa do NPC Celsus Park.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("celsus_park");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!celsus_park ficha" || texto === "!celsus_park ficha completa") {
        let mensagem = `*═══ FICHA DE CELSUS PARK ═══*
`;
        mensagem += `──────────────────────────
