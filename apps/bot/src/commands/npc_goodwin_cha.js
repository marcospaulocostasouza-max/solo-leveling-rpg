 ...
 * COMANDO: !goodwin_cha
 * 
 * Exibe a ficha completa do NPC Goodwin Cha.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("goodwin_cha");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!goodwin_cha ficha" || texto === "!goodwin_cha ficha completa") {
        let mensagem = `*═══ FICHA DE GOODWIN CHA ═══*
`;
        mensagem += `──────────────────────────
