 ...
 * COMANDO: !heidne_ahn
 * 
 * Exibe a ficha completa do NPC Heidne Ahn.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("heidne_ahn");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!heidne_ahn ficha" || texto === "!heidne_ahn ficha completa") {
        let mensagem = `*═══ FICHA DE HEIDNE AHN ═══*
`;
        mensagem += `──────────────────────────
