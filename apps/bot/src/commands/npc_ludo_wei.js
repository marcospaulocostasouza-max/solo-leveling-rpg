 ...
 * COMANDO: !ludo_wei
 * 
 * Exibe a ficha completa do NPC Ludo Wei.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("ludo_wei");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!ludo_wei ficha" || texto === "!ludo_wei ficha completa") {
        let mensagem = `*═══ FICHA DE LUDO WEI ═══*
`;
        mensagem += `──────────────────────────
