...
/**
 * COMANDO: !phenn_doyoung
 * 
 * Exibe a ficha completa do NPC Phenn Doyoung.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("phenn_doyoung");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!phenn_doyoung ficha" || texto === "!phenn_doyoung ficha completa") {
        let mensagem = `*═══ FICHA DE PHENN DOYOUNG ═══*
`;
        mensagem += `──────────────────────────
