 ...
 * COMANDO: !sazantos_do
 * 
 * Exibe a ficha completa do NPC Sazantos Do.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("sazantos_do");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!sazantos_do ficha" || texto === "!sazantos_do ficha completa") {
        let mensagem = `*═══ FICHA DE SAZANTOS DO ═══*
`;
        mensagem += `──────────────────────────
