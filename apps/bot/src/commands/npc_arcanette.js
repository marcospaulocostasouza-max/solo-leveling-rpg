 ...
 * COMANDO: !arcanette
 * 
 * Exibe a ficha completa do NPC Arcanette.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("arcanette");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!arcanette ficha" || texto === "!arcanette ficha completa") {
        let mensagem = `*═══ FICHA DE ARCANETTE ═══*
`;
        mensagem += `──────────────────────────
