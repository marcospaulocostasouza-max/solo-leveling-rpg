 ...
 * COMANDO: !yvon_baik
 * 
 * Exibe a ficha completa do NPC Yvon Baik.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("yvon_baik");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!yvon_baik ficha" || texto === "!yvon_baik ficha completa") {
        let mensagem = `*═══ FICHA DE YVON BAIK ═══*
`;
        mensagem += `──────────────────────────
