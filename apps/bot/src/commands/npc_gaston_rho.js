 ...
 * COMANDO: !gaston_rho
 * 
 * Exibe a ficha completa do NPC Gaston Rho.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("gaston_rho");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!gaston_rho ficha" || texto === "!gaston_rho ficha completa") {
        let mensagem = `*═══ FICHA DE GASTON RHO ═══*
`;
        mensagem += `──────────────────────────
