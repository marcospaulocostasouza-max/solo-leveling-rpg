 ...
 * COMANDO: !reime_oh
 * 
 * Exibe a ficha completa do NPC Reime Oh.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("reime_oh");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!reime_oh ficha" || texto === "!reime_oh ficha completa") {
        let mensagem = `*═══ FICHA DE REIME OH ═══*
`;
        mensagem += `──────────────────────────
