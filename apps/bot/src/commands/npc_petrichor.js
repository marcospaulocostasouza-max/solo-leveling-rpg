 ...
 * COMANDO: !petrichor
 * 
 * Exibe a ficha completa do NPC Petrichor.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("petrichor");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!petrichor ficha" || texto === "!petrichor ficha completa") {
        let mensagem = `*═══ FICHA DE PETRICHOR ═══*
`;
        mensagem += `──────────────────────────
