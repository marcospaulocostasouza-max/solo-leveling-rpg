...
async function ensure() {
  if (!ready) ready = database.run(`CREATE TABLE IF NOT EXISTS appearance_changes (
    ...
    quantidade INTEGER NOT NULL DEFAULT 0 CHECK(quantidade BETWEEN 0 AND 2))`).catch(error => { ready = null; throw error; });
  await ready;
...
async function change(playerId, input) {
  ...
  if (!appearance || appearance.length > 1000) return { erro: "Informe a aparência (até 1000 caracteres)." };
  await ensure();
