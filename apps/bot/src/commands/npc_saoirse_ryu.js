 ...
 * COMANDO: !saoirse_ryu
 * 
 * Exibe a ficha completa do NPC Saoirse Ryu.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("saoirse_ryu");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!saoirse_ryu ficha" || texto === "!saoirse_ryu ficha completa") {
        let mensagem = `*═══ FICHA DE SAOIRSE RYU ═══*
`;
        mensagem += `──────────────────────────
