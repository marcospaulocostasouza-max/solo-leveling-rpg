...
function extractConstraints(request) {
    ...
    const requestedAttribute = text.match(/(\d+)\s+(?:(?:pontos?\s+)?de\s+)?atributo\s+em\s+([\p{L}_ ]+)/iu);
    if (requestedAttribute) {
        const attribute = attributeName(requestedAttribute[2]);
        if (attribute) {
            constraints.attribute_total = Number(requestedAttribute[1]);
            constraints.attribute_distribution = { [attribute]: Number(requestedAttribute[1]) };
        }
    }
...
function attributeName(value) { const normalized = normalize(value).trim(); return ({ forca: "forca", resistencia: "resistencia", velocidade: "velocidade", sentidos: "sentidos", inteligencia: "inteligencia", "poder magico": "poder_magico" })[normalized] || null; }
...
module.exports = { TYPE_ALIASES, RULES, detectType, extractConstraints, createPlan, attributeName };
