 ...
 * COMANDO: !redeye
 * 
 * Exibe a ficha completa do NPC Redeye.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("redeye");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!redeye ficha" || texto === "!redeye ficha completa") {
        let mensagem = `*═══ FICHA DE REDEYE ═══*
`;
        mensagem += `──────────────────────────
