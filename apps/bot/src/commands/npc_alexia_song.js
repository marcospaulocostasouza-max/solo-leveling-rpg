 ...
 * COMANDO: !alexia_song
 * 
 * Exibe a ficha completa do NPC Alexia Song.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("alexia_song");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!alexia_song ficha" || texto === "!alexia_song ficha completa") {
        let mensagem = `*═══ FICHA DE ALEXIA SONG ═══*
`;
        mensagem += `──────────────────────────
