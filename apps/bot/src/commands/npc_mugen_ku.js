 ...
 * COMANDO: !mugen_ku
 * 
 * Exibe a ficha completa do NPC Mugen Ku.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("mugen_ku");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!mugen_ku ficha" || texto === "!mugen_ku ficha completa") {
        let mensagem = `*═══ FICHA DE MUGEN KU ═══*
`;
        mensagem += `──────────────────────────
