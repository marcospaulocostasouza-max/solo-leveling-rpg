...
/**
 * COMANDO: !alaune_yeong
 * 
 * Exibe a ficha completa do NPC Alaune Yeong.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("alaune_yeong");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!alaune_yeong ficha" || texto === "!alaune_yeong ficha completa") {
        let mensagem = `*═══ FICHA DE ALAUNE YEONG ═══*
`;
        mensagem += `──────────────────────────
