...
module.exports = async msg => {
    try {
        const pedidoItem = String(msg.body || "").replace(/^!equipar/i, "").trim();
...
_Use !inventario para ver seus itens._` });
        ...
        const resultado = await InventorySystem.equiparItem(jogador.id, item.id, item.inv_id);
