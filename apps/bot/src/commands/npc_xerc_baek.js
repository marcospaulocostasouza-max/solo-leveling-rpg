 ...
 * COMANDO: !xerc_baek
 * 
 * Exibe a ficha completa do NPC Xerc Baek.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("xerc_baek");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!xerc_baek ficha" || texto === "!xerc_baek ficha completa") {
        let mensagem = `*═══ FICHA DE XERC BAEK ═══*
`;
        mensagem += `──────────────────────────
