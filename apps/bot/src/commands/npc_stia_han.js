 ...
 * COMANDO: !stia_han
 * 
 * Exibe a ficha completa do NPC Stia Han.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("stia_han");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!stia_han ficha" || texto === "!stia_han ficha completa") {
        let mensagem = `*═══ FICHA DE STIA HAN ═══*
`;
        mensagem += `──────────────────────────
