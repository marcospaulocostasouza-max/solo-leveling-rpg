 ...
 * COMANDO: !tatloch
 * 
 * Exibe a ficha completa do NPC Tatloch.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("tatloch");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!tatloch ficha" || texto === "!tatloch ficha completa") {
        let mensagem = `*═══ FICHA DE TATLOCH ═══*
`;
        mensagem += `──────────────────────────
