 ...
 * COMANDO: !tanzy_woo
 * 
 * Exibe a ficha completa do NPC Tanzy Woo.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("tanzy_woo");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!tanzy_woo ficha" || texto === "!tanzy_woo ficha completa") {
        let mensagem = `*═══ FICHA DE TANZY WOO ═══*
`;
        mensagem += `──────────────────────────
