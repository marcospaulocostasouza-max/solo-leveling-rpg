 ...
 * COMANDO: !werner_choi
 * 
 * Exibe a ficha completa do NPC Werner Choi.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("werner_choi");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!werner_choi ficha" || texto === "!werner_choi ficha completa") {
        let mensagem = `*═══ FICHA DE WERNER CHOI ═══*
`;
        mensagem += `──────────────────────────
