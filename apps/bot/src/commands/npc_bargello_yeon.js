...
/**
 * COMANDO: !bargello_yeon
 * 
 * Exibe a ficha completa do NPC Bargello Yeon.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("bargello_yeon");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!bargello_yeon ficha" || texto === "!bargello_yeon ficha completa") {
        let mensagem = `*═══ FICHA DE BARGELLO YEON ═══*
`;
        mensagem += `──────────────────────────
