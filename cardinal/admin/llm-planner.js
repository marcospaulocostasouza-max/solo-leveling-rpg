...
async function planBatchWithModel(client, message, registry) {
    ...
    const interpretationRules = `Leia a ordem pelo significado, mesmo quando começar com "quero", "preciso", "poderia" ou "coloca". Interprete sinônimos, abreviações e texto cotidiano. Diferencie adicionar de definir o saldo final e retirar de substituir. Preserve todas as tarefas; nunca retorne somente parte de uma ordem. Uma pergunta informativa não é autorização para alterar. Não invente quantidades, objetos ou destinos. Se houver duas interpretações possíveis, peça esclarecimento em missing e retorne actions vazio. Não considere "sim" dentro da ordem como confirmação. O ADM vai revisar o plano antes de qualquer alteração.\n`;
    const answer = await client.chat(interpretationRules + prompt, { maxTokens: 2400, temperature: 0, responseFormat: { type: "json_object" } });
