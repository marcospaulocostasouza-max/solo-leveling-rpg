...
function planNatural(text) { const raw = String(text || "").trim(); const normalized = raw.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase(); let tool, parameters = {};
    let match;
    if ((match = raw.match(/^(?:altere|mude|troque|ajuste|defina)\s+(?:o|a)?\s*(nome|classe|estilo|rank|n[ií]vel|for[cç]a|resist[eê]ncia|velocidade|sentidos|intelig[eê]ncia|poder m[aá]gico)\s+(?:de|do|da)\s+(?:jogador\s+|player\s+)?(.+?)\s+para\s+(.+?)[.!]*$/i))) { tool = "edit_player"; const field = { nome: "nome", classe: "classe", estilo: "estilo_luta", rank: "rank", nivel: "nivel", forca: "forca_base", resistencia: "resistencia_base", velocidade: "velocidade_base", sentidos: "sentidos_base", inteligencia: "inteligencia_base", "poder magico": "poder_magico_base" }[match[1].normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()]; parameters = { player: match[2], changes: { [field]: (field === "nivel" || field.endsWith("_base")) ? Number(match[3]) : match[3] } }; }
    else if ((match = normalized.match(/^(?:desative|desativar)\s+(?:o\s+)?banner\s+(.+)$/))) { tool = "deactivate_banner"; parameters = { banner: match[1] }; }
    else if ((parameters = itemGrant(normalized))) { tool = "give_item"; }
