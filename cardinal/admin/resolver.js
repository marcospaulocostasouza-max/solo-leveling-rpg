...
class EntityResolver {
    ...
    async resolve(table, value, options = {}) {
        ...
        if (!rows.length) {
            const candidates = await this.database.all(`SELECT id,${field} FROM ${table}`);
            const matches = candidates.filter(row => normalize(row[field]).includes(normalize(value)));
            if (matches.length > 1) throw new AdminError(CODES.AMBIGUOUS_TARGET, `Alvo ambíguo: ${value}.`, { matches });
            if (matches.length === 1) return this.database.get(`SELECT * FROM ${table} WHERE id=?`, [matches[0].id]);
        }
