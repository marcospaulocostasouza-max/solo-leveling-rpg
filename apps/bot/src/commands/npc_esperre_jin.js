 ...
 * COMANDO: !esperre_jin
 * 
 * Exibe a ficha completa do NPC Esperre Jin.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("esperre_jin");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!esperre_jin ficha" || texto === "!esperre_jin ficha completa") {
        let mensagem = `*═══ FICHA DE ESPERRE JIN ═══*
`;
        mensagem += `──────────────────────────
