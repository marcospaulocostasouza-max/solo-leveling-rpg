...
class AdminRepository {
    ...
    async createConfirmation(adminNumber, operation, ttlMs = 300000, confirmationId = null) { await this.initialize(); const id = confirmationId || `confirm_${crypto.randomUUID()}`, now = new Date(), expires = new Date(now.getTime() + ttlMs); await this.database.run("INSERT INTO cardinal_admin_confirmations(confirmation_id,operation_json,admin_number,expires_at,created_at) VALUES(?,?,?,?,?) ON CONFLICT(confirmation_id) DO NOTHING", [id, JSON.stringify(operation), adminNumber, expires.toISOString(), now.toISOString()]); return { confirmation_id: id, expires_at: expires.toISOString() }; }
    async consumeConfirmation(id, adminNumber) { await this.initialize(); return this.database.transaction(query => this.consumeConfirmationWithQuery(query, id, adminNumber)); }
    async consumeConfirmationWithQuery(query, id, adminNumber) {
        const row = await query.get("SELECT * FROM cardinal_admin_confirmations WHERE confirmation_id=?", [id]);
        if (!row || row.admin_number !== adminNumber) throw new AdminError(CODES.CONFIRMATION_EXPIRED, "Confirmação inexistente ou não pertence a este administrador.");
        const now = new Date().toISOString();
        if (row.confirmed_at || Date.parse(row.expires_at) <= Date.now()) throw new AdminError(CODES.CONFIRMATION_EXPIRED, "A confirmação expirou ou já foi utilizada.");
        const consumed = await query.run("UPDATE cardinal_admin_confirmations SET confirmed_at=? WHERE confirmation_id=? AND admin_number=? AND confirmed_at IS NULL AND expires_at>?", [now, id, adminNumber, now]);
        if (consumed.changes !== 1) throw new AdminError(CODES.CONFIRMATION_EXPIRED, "A confirmação já foi utilizada ou expirou.");
        return JSON.parse(row.operation_json);
    }
}
