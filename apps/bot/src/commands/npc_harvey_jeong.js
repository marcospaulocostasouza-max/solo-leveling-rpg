...
/**
 * COMANDO: !harvey_jeong
 * 
 * Exibe a ficha completa do NPC Harvey Jeong.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("harvey_jeong");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!harvey_jeong ficha" || texto === "!harvey_jeong ficha completa") {
        let mensagem = `*═══ FICHA DE HARVEY JEONG ═══*
`;
        mensagem += `──────────────────────────
