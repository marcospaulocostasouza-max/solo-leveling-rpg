 ...
 * COMANDO: !claude
 * 
 * Exibe a ficha completa do NPC Claude.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("claude");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!claude ficha" || texto === "!claude ficha completa") {
        let mensagem = `*═══ FICHA DE CLAUDE ═══*
`;
        mensagem += `──────────────────────────
