 ...
 * COMANDO: !simeon_ha
 * 
 * Exibe a ficha completa do NPC Simeon Ha.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("simeon_ha");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!simeon_ha ficha" || texto === "!simeon_ha ficha completa") {
        let mensagem = `*═══ FICHA DE SIMEON HA ═══*
`;
        mensagem += `──────────────────────────
