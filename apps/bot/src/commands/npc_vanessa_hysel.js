...
/**
 * COMANDO: !vanessa_hysel
 * 
 * Exibe a ficha completa do NPC Vanessa Hysel.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("vanessa_hysel");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!vanessa_hysel ficha" || texto === "!vanessa_hysel ficha completa") {
        let mensagem = `*═══ FICHA DE VANESSA HYSEL ═══*
`;
        mensagem += `──────────────────────────
