...
/**
 * COMANDO: !delitia_song
 * 
 * Exibe a ficha completa do NPC Delitia Song.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("delitia_song");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!delitia_song ficha" || texto === "!delitia_song ficha completa") {
        let mensagem = `*═══ FICHA DE DELITIA SONG ═══*
`;
        mensagem += `──────────────────────────
