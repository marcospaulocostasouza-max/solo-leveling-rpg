...
/**
 * COMANDO: !carinda_moon
 * 
 * Exibe a ficha completa do NPC Carinda Moon.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("carinda_moon");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!carinda_moon ficha" || texto === "!carinda_moon ficha completa") {
        let mensagem = `*═══ FICHA DE CARINDA MOON ═══*
`;
        mensagem += `──────────────────────────
