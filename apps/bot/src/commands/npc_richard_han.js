 ...
 * COMANDO: !richard_han
 * 
 * Exibe a ficha completa do NPC Richard Han.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("richard_han");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!richard_han ficha" || texto === "!richard_han ficha completa") {
        let mensagem = `*═══ FICHA DE RICHARD HAN ═══*
`;
        mensagem += `──────────────────────────
