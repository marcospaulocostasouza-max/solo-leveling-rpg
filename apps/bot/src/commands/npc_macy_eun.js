 ...
 * COMANDO: !macy_eun
 * 
 * Exibe a ficha completa do NPC Macy Eun.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("macy_eun");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!macy_eun ficha" || texto === "!macy_eun ficha completa") {
        let mensagem = `*═══ FICHA DE MACY EUN ═══*
`;
        mensagem += `──────────────────────────
