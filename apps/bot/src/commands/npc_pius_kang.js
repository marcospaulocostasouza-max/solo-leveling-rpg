 ...
 * COMANDO: !pius_kang
 * 
 * Exibe a ficha completa do NPC Pius Kang.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("pius_kang");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!pius_kang ficha" || texto === "!pius_kang ficha completa") {
        let mensagem = `*═══ FICHA DE PIUS KANG ═══*
`;
        mensagem += `──────────────────────────
