 ...
 * COMANDO: !lucia_yeom
 * 
 * Exibe a ficha completa do NPC Lucia Yeom.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("lucia_yeom");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!lucia_yeom ficha" || texto === "!lucia_yeom ficha completa") {
        let mensagem = `*═══ FICHA DE LUCIA YEOM ═══*
`;
        mensagem += `──────────────────────────
