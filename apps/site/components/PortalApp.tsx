...
import { classAttributeBonus } from '../lib/class-attribute-bonus';

type Tab = 'inicio'|'personagem'|'inventario'|'equipamentos'|'habilidades'|'missoes'|'guilda'|'loja'|'sistemas'|'npcs'|'ranking';
...
function DemoPortalApp(){
  ...
  const nav=useMemo(()=>[
    ['inicio','Início',Home],['personagem','Meu Personagem',UserRound],['inventario','Inventário',Backpack],['equipamentos','Equipamentos',Shield],['habilidades','Habilidades',Zap],['missoes','Missões',ScrollText],['guilda','Guildas',Crown],['loja','Loja',ShoppingBag],['sistemas','Sistemas',BookOpen],['npcs','NPCs',Users],['ranking','Rankings',Trophy]
  ] as [Tab,string,any][],[]);
  return <main className="portal-shell"><aside className={menu?'portal-nav open':'portal-nav'}><div className="brand"><div className="brand-mark">SL</div><div><b>SOLO LEVELING</b><span>RPG SYSTEM</span></div><button onClick={()=>setMenu(false)}><X/></button></div><nav>{nav.map(([id,label,Icon])=><button key={id} onClick={()=>{setTab(id);setMenu(false)}} className={tab===id?'active':''}><Icon/><span>{label}</span></button>)}<Link href="/mapa" onClick={()=>setMenu(false)}><Map/><span>Mapa</span></Link></nav><div className="nav-footer"><DoorOpen/><span>!site • sessão vinculada</span></div></aside><section className="portal-content"><header className="topbar"><button className="menu-btn" onClick={()=>setMenu(true)}><Menu/></button><div><span>LOCALIZAÇÃO</span><b>📍 Seul</b></div><div className="top-character"><span>{character.name}</span><b>Lv. {character.level} • Rank {character.rank}</b></div></header><div className="content-body">{tab==='inicio'&&<HomeView setTab={setTab}/>} {tab==='personagem'&&<CharacterView avatar={avatar} setAvatar={setAvatar}/>} {tab==='inventario'&&<InventoryView/>} {tab==='equipamentos'&&<EquipmentView avatar={avatar}/>} {tab==='habilidades'&&<SkillsView/>} {tab==='missoes'&&<QuestView/>} {tab==='guilda'&&<GuildView/>} {tab==='loja'&&<ShopView/>} {tab==='sistemas'&&<SystemsView/>} {tab==='npcs'&&<NpcView/>} {tab==='ranking'&&<RankingView/>}</div></section></main>
}
...
function LivePortal(){
 const [data,setData]=useState<LivePayload|null>(null); const [tab,setTab]=useState<'personagem'|'inventario'|'equipamentos'|'habilidades'|'missoes'|'npcs'|'loja'>('personagem'); const [error,setError]=useState(''); const [busy,setBusy]=useState<string|number|null>(null); const [shop,setShop]=useState<any[]>([]); const [quests,setQuests]=useState<any[]>([]); const [npcs,setNpcs]=useState<any[]>([]);
