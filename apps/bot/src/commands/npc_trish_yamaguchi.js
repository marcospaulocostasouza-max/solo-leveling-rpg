...
/**
 * COMANDO: !trish_yamaguchi
 * 
 * Exibe a ficha completa do NPC Trish Yamaguchi.
 */
...
module.exports = async (msg) => {
    ...
    const npc = NPCManager.carregarNPC("trish_yamaguchi");
    
    if (!npc) {
        return MessageService.send({ message: msg, text: "*✖ NPC não encontrado.*" });
    }
    
    if (texto === "!trish_yamaguchi ficha" || texto === "!trish_yamaguchi ficha completa") {
        let mensagem = `*═══ FICHA DE TRISH YAMAGUCHI ═══*
`;
        mensagem += `──────────────────────────
